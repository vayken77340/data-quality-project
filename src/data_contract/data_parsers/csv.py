...
class CsvParser(FileParser):
    ...
    Schema:         column_names exposed from the header row. column_types
                    is None -- CSV doesn't carry type information.
    """
    ...
    def parse_file(
        ...
        # Column names come from the header row. CSV doesn't carry types.
        column_names = lf.collect_schema().names()
        schema = ParserSchema(column_names=column_names, column_types=None)
        return ParsedFile(rows=lf, schema=schema)
