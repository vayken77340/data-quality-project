"""Post-check builders + per-table diagnostics.
...
artifacts every report format expects:
  ...
  * `diagnose_missing_inputs` -- friendly "no files matched" explanation.
...
from data_contract.validation.config import Gates, ValidationConfig
...
# ---------------------------------------------------------------------------
# Diagnostic for the "no input files" violation
# ---------------------------------------------------------------------------
...
def populate_by_check(report: TableReport, checks: Gates) -> None:
    ...
    from data_contract.validation.report.check_status import compute_table_check_status
    from data_contract.validation.report.dimensions import _CHECK_TO_VIOLATION_KIND
