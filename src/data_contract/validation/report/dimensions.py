...
* **Table-level** (`pk_uniqueness`, `fk_existence`, `column_missing`, ...)
  -- pluggable via the `data_contract.table_checks/` registry. Each
...
* **Per-constraint** (`allowed_values`, `pattern`, `unique`, ...) --
  pluggable via the `data_contract.field_constraints/` registry. Each
...
def violation_kind_for_check(check_name: str) -> str:
    ...
    try:
        ...
        raise KeyError(
            ...
            f"data_contract.table_checks or data_contract.field_constraints, "
