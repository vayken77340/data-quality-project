...
def test_csv_single_file_row_count_and_source_columns(tmp_path, csv_params):
    p = _write_csv(tmp_path / "a.csv", "id,name\n1,alpha\n2,beta\n3,gamma\n")
    parser = CsvParser(csv_params())
    lf = parser.read([p]).frame
    df = lf.collect()
    assert df.height == 3
...
def test_csv_multi_file_concat_attributes_source(tmp_path, csv_params):
    p1 = _write_csv(tmp_path / "jan.csv", "id,name\n1,a\n2,b\n")
    p2 = _write_csv(tmp_path / "feb.csv", "id,name\n3,c\n4,d\n")
    parser = CsvParser(csv_params())
    df = parser.read([p1, p2]).frame.collect()
    assert df.height == 4
...
def test_csv_custom_delimiter(tmp_path, csv_params):
    p = _write_csv(tmp_path / "semi.csv", "id;name\n1;a\n2;b\n")
    parser = CsvParser(csv_params(**{"delimiter": ";"}))
...
def test_csv_null_tokens_become_nulls(tmp_path, csv_params):
    p = _write_csv(tmp_path / "n.csv", "id,note\n1,NULL\n2,present\n3,\n")
    parser = CsvParser(csv_params(**{"null_tokens": ["", "NULL"]}))
    df = parser.read([p]).frame.collect()
    notes = df["note"].to_list()
    assert notes == [None, "present", None]


def test_csv_header_row_offset(tmp_path, csv_params):
    p = _write_csv(
        tmp_path / "headered.csv",
        "preface line\nblank\nid,name\n1,a\n2,b\n",
    )
    parser = CsvParser(csv_params(**{"header_row": 3}))
    df = parser.read([p]).frame.collect()
    assert df.height == 2
...
def test_csv_all_data_columns_are_string_dtype(tmp_path, csv_params):
    ...
    df = CsvParser(csv_params()).read([p]).frame.collect()
    for col in ("i", "f", "d", "b", "s"):
        assert df.schema[col] == pl.String, f"column {col!r} dtype is {df.schema[col]!r}"


def test_csv_leading_zero_preserved(tmp_path, csv_params):
...
def test_csv_french_comma_preserved_for_contract_layer(tmp_path, csv_params):
    ...
    df = CsvParser(csv_params(**{"delimiter": ";"})).read([p]).frame.collect()
