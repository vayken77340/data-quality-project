"""Behavioural tests for the PK uniqueness + FK existence TableCheck classes."""
...
# ---------------------------------------------------------------------------
# FK existence (cross-table; uses the per-field helper)
# ---------------------------------------------------------------------------
...
def test_fk_existence_flags_dangling_reference():
    ...
    violating = FkExistenceCheck.run_for_field(child, "parent_id", parent, "pk")
    rows = violating.collect().to_dicts()
...
def test_fk_existence_does_not_flag_null():
    ...
    # No violations -> None.
    assert FkExistenceCheck.run_for_field(child, "parent_id", parent, "pk") is None
