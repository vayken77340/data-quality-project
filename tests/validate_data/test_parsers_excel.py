...
from data_contract.validate_data.parsers.excel import ExcelParser, _to_string
...
def test_excel_all_data_columns_are_string_dtype(tmp_path):
    ...
    df = ExcelParser().read([p]).collect()
    for col in ("i", "f", "d", "b", "s"):
        assert df.schema[col] == pl.String, f"{col!r} dtype is {df.schema[col]!r}"


def test_excel_whole_number_cell_stringifies_without_trailing_zero(tmp_path):
...
def test_excel_fractional_float_stringifies_as_float(tmp_path):
    p = _write_xlsx(tmp_path / "f.xlsx", [["x"], [1.5], [3.14]])
    df = ExcelParser().read([p]).collect()
    assert df["x"].to_list() == ["1.5", "3.14"]


def test_excel_date_cell_stringifies_to_iso(tmp_path):
...
def test_excel_datetime_cell_stringifies_with_space_separator(tmp_path):
...
def test_excel_bool_cell_stringifies_to_python_default(tmp_path):
...
def test_excel_text_formatted_cell_passes_through_verbatim(tmp_path):
...
def test_excel_null_tokens_mapping(tmp_path):
    ...
    df = ExcelParser({"null_tokens": ["", "NULL"]}).read([p]).collect()
