...
def test_json_table_payload_carries_by_check_and_metrics(tmp_path: Path):
    """Every per-table payload exposes the new by_check grid + metrics dict."""
    p = _run(tmp_path, "id,label\n1,abc\n2,def\n")
    tbl = p["tables"][0]
    assert "by_check" in tbl
    assert "metrics" in tbl
    # by_check entries have the expected shape (status + counts + scope).
    bc = tbl["by_check"]
    assert "nullable" in bc
    nullable_status = bc["nullable"]
    assert set(nullable_status) >= {
        "status", "scope", "pass_rows", "warning_rows",
        "error_rows", "violation_count",
    }
    # All-pass clean run -> every row-scope check reports OK with full pass count.
    assert nullable_status["status"] == "OK"
    assert nullable_status["pass_rows"] == 2
    # Metrics dict carries enabled metrics with their values.
    metrics = tbl["metrics"]
    assert "null_count" in metrics
    assert metrics["null_count"]["scope"] == "field"
    assert metrics["null_count"]["values"] == {"id": 0, "label": 0}
    assert metrics["row_count"]["scope"] == "table"
    assert metrics["row_count"]["values"] == {"__table__": 2}


def test_json_by_check_marks_error_for_max_length_violation(tmp_path: Path):
    p = _run(tmp_path, "id,label\n1,toolong\n")   # 'toolong' violates max_length=3
    tbl = p["tables"][0]
    status = tbl["by_check"]["max_length"]
    assert status["status"] == "ERROR"
    assert status["error_rows"] == 1
    assert status["pass_rows"] == 0
...
# ---------------------------------------------------------------------------
# Table-level (no source row) violations route to run_issues, not Top issues
# ---------------------------------------------------------------------------


def test_column_missing_routes_to_run_issues(tmp_path: Path):
    """`column_missing` is table-level: it must surface in `run_issues` (with
    check_label + dimension + hint) and stay OUT of `tables[].violations`
    (which is for row-aggregated Top issues)."""
    p = _run(tmp_path, "id\n1\n")     # CSV missing the contract's `label` column.
    issues = [i for i in p["run_issues"] if i["kind"] == "column_missing"]
    assert len(issues) == 1
    i = issues[0]
    assert i["table"] == "T"
    assert i["field"] == "label"
