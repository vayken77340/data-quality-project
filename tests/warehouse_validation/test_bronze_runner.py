...
def test_diverging_bronze_name_matches_bronze_schema_and_quotes_in_try_cast(
    ...
    write_contract_yaml(
        ...
        field_blocks=[
            ...
            # back to f.silver_name for it (covers both branches of `_bronze_col`).
            (
                '  - silver_name: amount\n'
                '    type: int64\n'
                '    nullable: true\n'
    ...
    # The clean field falls back to f.silver_name via _bronze_col.
...
def test_silver_used_when_extract_and_bronze_both_unset(
    ...
    """Regression: when neither bronze_name nor extract_name is set in the
    spec, always-emit materializes both to silver and the bronze runner
    quotes the silver identifier. The old runtime cascade
    (`bronze_name or extract_name or f.silver_name`) is dead -- always-emit bakes
    the resolution in at build time, and `_bronze_col` just reads
    `f.bronze_name`."""
    epic_root = tmp_path / "epics"
    write_contract_yaml(
